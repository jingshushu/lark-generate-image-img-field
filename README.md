...
- 图片生成通常超过飞书同步执行的 58 秒限制，发布时必须选择「异步执行」（最长等待 15 分钟）。
...
运行日志应出现 `yunwu_direct_response` 和 `generate_image_img / complete`。如果只有
`yunwu_direct_request`，说明请求仍在等待云雾响应，或被飞书执行时限终止。
